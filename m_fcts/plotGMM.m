...
% 	R = sqrtm(Sigma(:,:,i));
	...
	if nargin>3 %Plot with alpha transparency
		patch(X(1,:,i), X(2,:,i), color, 'lineWidth', 1, 'EdgeColor', color, 'facealpha', valAlpha,'edgealpha', valAlpha);
        hold on
		%MuTmp = [cos(t); sin(t)] * 0.3 + repmat(Mu(:,i),1,nbDrawingSeg);
		%h = [h patch(MuTmp(1,:), MuTmp(2,:), darkcolor, 'LineStyle', 'none', 'facealpha', valAlpha)];
		plot(Mu(1,:), Mu(2,:), '.', 'markersize', 10, 'color', darkcolor);
	else %Plot without transparency
		%Standard plot
		patch(X(1,:,i), X(2,:,i), color, 'lineWidth', 1, 'EdgeColor', darkcolor);hold on
		plot(Mu(1,:), Mu(2,:), '.', 'markersize', 10, 'color', darkcolor);
% 		%Plot only contours
